...
| `allow_inherit` | A boolean indicating that a parent element can also have this styling and pass it down onto the child instead.  | | `False`, which will require the element itself to have this property. |
...
div_tag.has_styling("background-color", allow_inherit=True)
...
```python
def has_color(prop: str, color: str, important: Optional[bool] = None, allow_inherit: bool = False) -> Check
```
...
div.has_color("background-color", "blue", allow_inherit=True)  # Allow inheriting from the parent <body>
```
