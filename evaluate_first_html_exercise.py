...
def create_suites(content: str) -> List[checks.TestSuite]:
    ...
    ci_table_exists = checks.ChecklistItem("The body has a table.", [el_table.exists()])
    ...
    ci_caption_is_correct = checks.ChecklistItem(
        "The caption is correct.", el_table.get_child("caption").has_content(caption_content)
    )
    ...
    ci_table_header_is_correct = checks.ChecklistItem(
        "The first row has a header with the required text.",
        [
    ...
    ci_second_row_is_correct = checks.ChecklistItem(
        "The second row contains the required data.", table_rows[1].table_row_has_content(second_row)
    )

    # Item 8: the third row is correct
    third_row = ["Harry Potter", "Luna Lovegood", "Hannah Longbottom", "Pansy Parkinson"]
    ci_third_row_is_correct = checks.ChecklistItem(
        "The third row contains the required data.", table_rows[2].table_row_has_content(third_row)
    )

    # Item 9: the fourth row is correct
    fourth_row = ["Ronald Weasley", "Cho Chang", "Susan Bones", "Gregory Goyle"]
    ci_fourth_row_is_correct = checks.ChecklistItem(
        "The fourth row contains the required data.", table_rows[3].table_row_has_content(fourth_row)
    )
